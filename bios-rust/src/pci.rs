...
pub const fn pci_off(bus: u8, dev: u8, func: u8, offset: u8) -> u32 {
    ((bus as u32) << 16)
        | ((dev as u32) << 11)
        | ((func as u32) << 8)
        | ((offset as u32) & 0xFC)
...
pub fn pci_read32(bus: u8, dev: u8, func: u8, offset: u8) -> u32 {
    let addr = 0x8000_0000 | pci_off(bus, dev, func, offset);
    unsafe {
        core::arch::asm!("out dx, eax", in("dx") 0xCF8u16, in("eax") addr);
        let val: u32;
        core::arch::asm!("in eax, dx", in("dx") 0xCFCu16, out("eax") val);
        val
    }
}
...
fn pci_read16(bus: u8, dev: u8, func: u8, offset: u8) -> u16 {
    let v = pci_read32(bus, dev, func, offset & 0xFC);
    (v >> ((offset & 0x02) as u32 * 8)) as u16
}

fn pci_write16(bus: u8, dev: u8, func: u8, offset: u8, val: u16) {
    let shift = (offset & 0x02) as u32 * 8;
    let mut v = pci_read32(bus, dev, func, offset & 0xFC);
    v &= !(0xFFFF << shift);
    v |= (val as u32) << shift;
    pci_write32(bus, dev, func, offset & 0xFC, v);
}

fn pci_read8(bus: u8, dev: u8, func: u8, offset: u8) -> u8 {
    let v = pci_read32(bus, dev, func, offset & 0xFC);
    (v >> ((offset & 0x03) as u32 * 8)) as u8
}
...
static mut MMIO_NEXT: u32 = 0x1000_0000;

pub fn pci_enable_bars(bus: u8, dev: u8, func: u8) {
    let cmd = pci_read16(bus, dev, func, 0x04);

    // Always ensure I/O, Memory, and Bus Master are enabled
    pci_write16(bus, dev, func, 0x04, cmd | 0x07);

    // If BARs were already assigned by firmware, don't re-assign
    ...
    while b < 6 {
        ...
        if is_64 {
            pci_write32(bus, dev, func, 0x10 + b * 4 + 4, 0);
            b += 1;
        }
...
fn probe_ahci(bus: u8, dev: u8, func: u8) {
    ...
    puts("      ABAR: 0x");
    print_hex(abar as u64, 8);
    putc(b'\n');

    let ghc = abar + 0x04;
    let ghc_ptr = ghc as *mut u32;
    unsafe {
        core::ptr::write_volatile(ghc_ptr, core::ptr::read_volatile(ghc_ptr) | 0x8000_0000);
        for _ in 0..100 {
            core::hint::spin_loop();
        }
    }
    ...
    let np = (cap & 0x1F) + 1;
    let pi = unsafe { core::ptr::read_volatile((abar + 0x0C) as *const u32) };
    ...
    for p in 0..np.min(32) {
        ...
        let port_base = abar + 0x100 + (p as u32) * 0x80;
