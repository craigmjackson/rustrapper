...
fn reg_read32(base: u64, reg: u64) -> u32 {
    unsafe { read_volatile((base + reg) as *const u32) }
}

fn reg_write32(base: u64, reg: u64, val: u32) {
    unsafe { write_volatile((base + reg) as *mut u32, val) }
}

fn e1000_read_mac(base: u64) -> [u8; 6] {
...
fn e1000_set_mac(base: u64, mac: &[u8; 6]) {
...
fn e1000_clear_multicast(base: u64) {
    for i in 0..128 {
        reg_write32(base, REG_MTA + (i as u64) * 4, 0);
    }
}

fn e1000_init(base: u64) -> bool {
    ...
    reg_write32(base, REG_RDBAL, &raw const RX_DESCS as u64 as u32);
    ...
    reg_write32(base, REG_TDBAL, &raw const TX_DESCS as u64 as u32);
...
fn e1000_send(base: u64, data: &[u8]) -> bool {
    if data.len() > 2048 {
        return false;
    }
    ...
    for _ in 0..2000000 {
        unsafe {
            if TX_DESCS.0[0].status & TX_STATUS_DD != 0 {
                break;
            }
        }
        core::hint::spin_loop();
...
fn dhcp_recv(base: u64, mac: &[u8; 6], xid: u32) -> Option<DhcpConfig> {
...
pub fn scan_network() {
    ...
    for dev in 0..32u8 {
        ...
        puts("    MMIO BAR0: 0x");
        print_hex(bar0 as u64, 16);
        putc(b'\n');

        if !e1000_init(bar0 as u64) {
            puts("    e1000 init failed\n");
            continue;
        }

        let mac = e1000_read_mac(bar0 as u64);
        puts("    MAC: ");
        print_mac(&mac);
        putc(b'\n');

        puts("    DHCP: ");
        match dhcp_run(bar0 as u64, &mac) {
            Some(cfg) => {
                puts("OK\n");
                puts("    IP: ");
                print_ip(&cfg.yiaddr);
