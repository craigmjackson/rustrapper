...
5. **`fetch()` builtin**: The host registers a TFTP download callback (`fetch_file`) on the Lua interpreter for PXE `.lua` scripts (via `lua::run_with_fetch`) and, in the interactive REPL, enables it only after the user runs the `dhcp` builtin (which calls `net::setup_fetch_context()` and wires up `fetch_file`). `fetch("file")` downloads `file` from the DHCP `next_server` and returns its byte count (or `nil` on failure), so scripts can pull multiple files (e.g. a kernel + initrd). Downloaded files are kept in host memory: `uefi/src/fetch.rs` uses a lazily-allocated 16 MB `AllocatePool` region split into 4 slots (`MAX_FETCH_FILES`); `arm64-bare/src/fetch.rs` uses a 16 MB static BSS buffer (no heap). Both are reset per script run by `set_context`, called right before `lua::run_with_fetch` in `pxe_boot_*` / `pxe_boot`, and by `net::setup_fetch_context()` when `dhcp` runs.
...
```bash
cargo test --workspace        # All 234 tests across all crates
...
cargo test --package lua      # 57 tests (lexer, parser, evaluator, global keyword, dhcp builtin, demo script output, REPL fetch/dhcp)
...
| `lua` | 57 | Lexer tokens, parser AST structure (if/elseif/else, while, repeat/until, numeric for, generic `for ... in`, function args, table fields), evaluator (arithmetic, comparisons, locals/globals, recursion, nested tables, concat, `print`), `break` (in while/numeric-for/generic-for, innermost-only, outside-loop errors), `global` keyword (init, nil default, shadowing bypass, scope visibility, syntax errors), `dhcp` builtin (success enables fetch, failure disables, bare vs call form, errors, type print), demo script exact output, `fetch()` with mock callback, REPL (prompt, echo/backspace, error handling, state persistence, help, global works, dhcp enables fetch, fetch works/fails) |
