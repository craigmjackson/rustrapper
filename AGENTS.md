...
- Supported subset: ints, strings, booleans, `nil`, locals/globals, `+ - * / %`, comparisons, `and/or/not`, `..` concat, `if/elseif/else`, `while`, numeric `for`, generic `for k [, v] in table do ... end` (iterates a table's key/value pairs — the `in` keyword), named `function`/`return`, tables (array/`name =`/`[expr]` fields, `t.key`, `t[key]`), `print()`, `--` comments. No closures, floats, `repeat`, `break`, or multiple assignment.
...
| `lua` | 51 | Lexer tokens, parser AST structure (if/elseif/else, while, numeric for, function args, table fields), evaluator (arithmetic, comparisons, locals/globals, recursion, nested tables, concat, `print`), `global` keyword (init, nil default, shadowing bypass, scope visibility, syntax errors), `dhcp` builtin (success enables fetch, failure disables, bare vs call form, errors, type print), demo script exact output, `fetch()` with mock callback, REPL (prompt, echo/backspace, error handling, state persistence, help, global works, dhcp enables fetch, fetch works/fails) |
