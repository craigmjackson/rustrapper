...
- Dispatches to `scan_storage_devices` or `net::scan_network_devices` based on the choice. The Lua Shell choice does NOT set up networking automatically: it registers the `dhcp` builtin (which calls `net::setup_fetch_context()` and enables `crate::fetch::fetch_file` on success), so `fetch()` works in the REPL after the user runs `dhcp`.
...
- `execute_pxe_file(system_table, image_handle, buffer, size, filename, puts)` — the PXE entry point (all three `pxe_boot_*` call sites in `net.rs`). If the bootfile name ends with `.lua`, runs it in place with `lua::run_with_fetch_load(data, crate::u16_putc, crate::fetch::fetch_file, crate::fetch::load_file)`; everything else falls through to `execute_file`. Mirrors `arm64-bare/src/net.rs::pxe_boot`, reusing the same `lua` crate.
...
- `load_file(name: &str, buf: &mut [u8]) -> Option<usize>` is the `LuaState.load` `dofile()` callback: same TFTP download but into the interpreter's buffer, returning the length (or `None`).
...
- Presents the `[1]/[2]/[3]` menu via `common::menu::show_menu`, reading from the PL011 UART (`uart::getc`), and dispatches to `scan::scan_devices` or `net::scan_network` based on the choice. The Lua Shell choice does NOT set up networking automatically: it registers the `dhcp` builtin (which calls `net::setup_fetch_context()` and enables `crate::fetch::fetch_file` on success), so `fetch()` works in the REPL after the user runs `dhcp`.
...
- `pxe_boot` TFTP-downloads the bootfile into an `Arm64MemorySink`; if the bootfile name ends with `.lua`, it calls `crate::fetch::set_context(base, mac, cfg)` and executes the downloaded buffer in place with `lua::run_with_fetch_load(data, putc, crate::fetch::fetch_file, crate::fetch::load_file)`, otherwise it hands the buffer to `loader::execute_file`.
...
- `set_context(base, mac, cfg)` records the e1000 BAR0 MMIO base, MAC, and `DhcpConfig` (src IP + `next_server`) and resets the slots. Called from `pxe_boot` right before `lua::run_with_fetch_load`.
...
- Presents the `[1]/[2]/[3]` menu via `common::menu::show_menu`, reading from serial COM1 (`serial::getc`), and dispatches to `scan::scan_devices(pci::detect_device)` or `net::scan_network()`. The Lua Shell choice does NOT set up networking automatically: it registers the `dhcp` builtin (which calls `net::setup_fetch_context()` and enables `crate::fetch::fetch_file` on success), so `fetch()` works in the REPL after the user runs `dhcp`.
...
- Supported subset: ints, strings, booleans, `nil`, locals/globals, `+ - * / %`, comparisons, `and/or/not`, `..` concat, `if/elseif/else`, `while`, `repeat ... until cond` (runs the body at least once; body locals are visible in the condition), `break` (only inside `while`/`repeat`/`for` loops — rejected at parse time outside one, and resets within function bodies), `goto name` / `::name::` labels (jump to a label in the same block or an enclosing block — out of a block but never into a nested one or across a function; labels resolve per block via a parser scope stack, so forward and backward jumps both work), numeric `for`, generic `for k [, v] in table do ... end` (iterates a table's key/value pairs — the `in` keyword), named `function`/`return`, tables (array/`name =`/`[expr]` fields, `t.key`, `t[key]`), `print()`, `--` comments. No closures, floats, or multiple assignment.
- Variable scope: assignment to a plain name (`x = v`) updates an existing local (innermost outwards) or creates a global. The `global name [= v]` keyword forces a write to the global table even when a local shadows `name` (analogous to `_G.name = v`); reads still prefer locals. `global name` without a value sets the global to `nil`.
...
- `run_with_fetch(data, putc, fetch)` / `LuaState::run_with_fetch` register a host `fetch` callback before running, and `run_with_fetch_load(data, putc, fetch, load)` / `LuaState::run_with_fetch_load` also register a `dofile()` loader callback. The host sets `LuaState.fetch: Option<fn(&str) -> Option<usize>>` (default `None`); the `fetch("file")` builtin calls it with the filename and returns the downloaded byte count as a Lua number, or `nil` on failure. `None` callback → runtime error `"fetch not available"`. The `dofile("file")` builtin (`Value::Native(2)`) uses `LuaState.load: Option<fn(&str, &mut [u8]) -> Option<usize>>` to load a Lua source chunk by name into a `DOFILE_CAP`-byte scratch buffer, then parses and runs it in the current state, returning the chunk's return value (`nil` if it doesn't return); errors in the chunk propagate to the caller. Value dispatch: `Value::Native(0)` = `print`, `Value::Native(1)` = `fetch`, `Value::Native(2)` = `dofile`, `Value::Shell` = `shell`, `Value::Dhcp` = `dhcp`, `Value::Exit` = `exit` (all registered as globals in `LuaState::run`). The `dhcp` builtin runs a host callback (`LuaState.dhcp`) that sets up the network and returns the `fetch` callback on success, so `fetch()` works interactively after the user types `dhcp` in the REPL — not just from PXE scripts. `dofile()` similarly needs a TFTP server: each host's `load_file` (in its `fetch.rs`) returns `None` until `dhcp` sets the network context.
...
4. **File Execution**: Bootloader detects file format via `common::loader::detect_format`:
   - **UEFI**: PE/COFF files → `LoadImage` + `StartImage` via Boot Services. `.lua` files → execute in place with the built-in Lua interpreter (`lua::run_with_fetch_load` with the host `fetch_file`/`load_file` callbacks) via `loader::execute_pxe_file`.
   ...
   - **ARM64 bare-metal**: `.lua` files → execute in place with the built-in Lua interpreter (`lua::run_with_fetch_load` with the host `fetch_file`/`load_file` callbacks).
   - **All targets**: Text files → display via `puts`. Binary files → print size only.
5. **`fetch()` / `dofile()` builtins**: The host registers TFTP download callbacks (`fetch_file` and `load_file`) on the Lua interpreter for PXE `.lua` scripts (via `lua::run_with_fetch_load`) and, in the interactive REPL, enables them after the user runs the `dhcp` builtin (which calls `net::setup_fetch_context()` and wires up the callbacks). `fetch("file")` downloads `file` from the DHCP `next_server` and returns its byte count (or `nil` on failure), so scripts can pull multiple files (e.g. a kernel + initrd). `dofile("file.lua")` loads a Lua source chunk, executes it in the current interpreter state, and returns the chunk's return value. Downloaded files are kept in host memory: `uefi/src/fetch.rs` uses a lazily-allocated 16 MB `AllocatePool` region split into 4 slots (`MAX_FETCH_FILES`); `arm64-bare/src/fetch.rs` uses a 16 MB static BSS buffer (no heap). Both are reset per script run by `set_context`, called right before `lua::run_with_fetch_load` in `pxe_boot_*` / `pxe_boot`, and by `net::setup_fetch_context()` when `dhcp` runs.
...
```bash
cargo test --workspace        # All 238 tests across all crates
...
cargo test --package lua      # 61 tests (lexer, parser, evaluator, global keyword, dhcp builtin, dofile builtin, demo script output, REPL fetch/dhcp)
...
| `lua` | 61 | Lexer tokens, parser AST structure (if/elseif/else, while, repeat/until, numeric for, generic `for ... in`, function args, table fields), evaluator (arithmetic, comparisons, locals/globals, recursion, nested tables, concat, `print`), `break` (in while/numeric-for/generic-for, innermost-only, outside-loop errors), `goto`/labels (forward/backward jumps, out of a block, continue-style, scoped per function, step-limit on goto loops, unknown-label/scope errors), `global` keyword (init, nil default, shadowing bypass, scope visibility, syntax errors), `dhcp` builtin (success enables fetch, failure disables, bare vs call form, errors, type print), `dofile` builtin (chunk return value, shared globals, chunk-defined functions, nested dofile, missing-file/runtime-error propagation, arg errors, no-loader error), demo script exact output, `fetch()` with mock callback, REPL (prompt, echo/backspace, error handling, state persistence, help, global works, dhcp enables fetch, fetch works/fails) |
