...
```bash
cargo test --workspace        # All 222 tests across all crates
...
cargo test --package lua      # 47 tests (lexer, parser, evaluator, global keyword, demo script output, REPL fetch)
...
| `lua` | 47 | Lexer tokens, parser AST structure (if/elseif/else, while, numeric for, function args, table fields), evaluator (arithmetic, comparisons, locals/globals, recursion, nested tables, concat, `print`), `global` keyword (init, nil default, shadowing bypass, scope visibility, syntax errors), demo script exact output, `fetch()` with mock callback, REPL (prompt, echo/backspace, error handling, state persistence, help, global works, fetch works/fails) |
